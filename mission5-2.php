<?php	//データベース接続
 ...
 if(!empty($_POST["submit_btn"])&&!empty($_POST["name"])&&!empty($_POST["comment"])&&!empty($_POST["submit_pass"])){
    ...
    if(empty($_POST["edit_post"])){//編集番号(プログラムが入力)が空のとき=新規投稿 
       $sql = $pdo -> prepare("INSERT INTO tb1 (name, comment,date,pass) VALUES (:name, :comment,:date,:pass)");
       //挿入先としてtb1の各列を指定、VALUEＳ以下で挿入する名前を指定
       $sql -> bindParam(':name', $name, PDO::PARAM_STR);//:nameには$nameを挿入
       $sql -> bindParam(':comment', $comment, PDO::PARAM_STR);//矢印は$sqlに対して行う操作
       $sql -> bindParam(':date', $date, PDO::PARAM_STR);//bindpramはexecuteまで()内をbindする
       $sql -> bindParam(':pass', $submit_pass, PDO::PARAM_STR);//文字列データ型、定義文
       $sql -> execute();//prepareのかたまりを実行
       ...
       $stmt -> bindParam(':pass', $submit_pass, PDO::PARAM_STR);
    ...
    if(!empty($_POST["edit_num"])&&!empty($_POST["edit_btn"])&&!empty($_POST["edit_pass"])){
        ...
        $stmt->execute(array(':id' => $edit_num,':pass'=>$edit_pass));
        $result = 0;
